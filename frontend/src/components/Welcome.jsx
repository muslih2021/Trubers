...
const Welcome = () => {
	...
	const {
		...
		selectedUuid
			? `${API_URL}/ContentReportPerforma/${selectedUuid}?themeId=${Theme}`
			: null,
		fetcher
	);
	const {
	...
	} = useSWR(`${API_URL}/allThemes`, fetcher);
		...
		<div className="min-w-full px-2 md:flex pb-8 md:px-6 justify-content-center gap-6">
			...
			<div className="flex-1 flex flex-column md:gap-4 gap-3">
				...
				<DetailModalUser
					...
					user={userDetail || ""}
					theme={Theme} // ✅ kirim state Theme ke modal
					themedata={themedata?.data || []}
					setTheme={setTheme} // ✅ opsional: kalau mau ganti Theme dari modal
				/>
			</div>
		</div>
	);
